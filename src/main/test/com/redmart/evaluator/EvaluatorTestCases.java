...
public class EvaluatorTestCases {
    ...
    @Test
    public void testWithOutCycleWithSingleInvalidExpression() {
        row = 2;
        col = 3;
        input.add("A2");
        input.add("4 5 *");
        ...
        input.add("A1 B2 / 2 +");
        input.add("3");
        input.add("39 B1 B2 *"); // Invalid expression
        try {
            sheetEvaluator = new SheetEvaluator(row, col, input);
            Assert.assertFalse(sheetEvaluator.isCyclicDependency());
            Assert.assertTrue(sheetEvaluator.getSheetDao().getSheet().getCell(1, 2).isInvalidData());
        } catch (Exception e) {
